...
if Config.MaxInService ~= -1 then
	TriggerEvent('::{korioz#0110}::esx_service:activateService', 'sheriff', Config.MaxInService)
end

TriggerEvent('::{korioz#0110}::esx_phone:registerNumber', 'sheriff', _U('alert_sheriff'), true, true)
TriggerEvent('::{korioz#0110}::esx_society:registerSociety', 'sheriff', 'Sheriff', 'society_sheriff', 'society_sheriff', 'society_sheriff', {type = 'public'})

RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:confiscatePlayerItem')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
				TriggerClientEvent('::{korioz#0110}::esx:showNotification', _source, _U('quantity_invalid'))
			else
...
RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:handcuff')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:handcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'sheriff' then
		TriggerClientEvent('::{korioz#0110}::esx_sheriffjob:handcuff', target)
	else
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:drag', function(target)
	...
	if xPlayer.job.name == 'sheriff' then
		TriggerClientEvent('::{korioz#0110}::esx_sheriffjob:drag', target, source)
	else
...
RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:getStockItem')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:getStockItem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sourceItem = xPlayer.getInventoryItem(itemName)

	TriggerEvent('::{korioz#0110}::esx_addoninventory:getSharedInventory', 'society_sheriff', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerClientEvent('::{korioz#0110}::esx:showNotification', _source, _U('have_withdrawn', count, inventoryItem.label))
			end
		else
			TriggerClientEvent('::{korioz#0110}::esx:showNotification', _source, _U('quantity_invalid'))
		end
...
RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:putStockItems')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
	...
	TriggerEvent('::{korioz#0110}::esx_addoninventory:getSharedInventory', 'society_sheriff', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerClientEvent('::{korioz#0110}::esx:showNotification', xPlayer.source, _U('have_deposited', count, inventoryItem.label))
		else
...
ESX.RegisterServerCallback('::{korioz#0110}::esx_sheriffjob:getOtherPlayerData', function(source, cb, target)
...
ESX.RegisterServerCallback('esx_sheriffjob:getArmoryWeapons', function(source, cb)
	TriggerEvent('::{korioz#0110}::esx_datastore:getSharedDataStore', 'society_sheriff', function(store)
...
AddEventHandler('::{korioz#0110}::playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'sheriff' then
			Citizen.Wait(5000)
			TriggerClientEvent('::{korioz#0110}::esx_sheriffjob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:spawned')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:forceBlip')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:forceBlip', function()
	TriggerClientEvent('::{korioz#0110}::esx_sheriffjob:updateBlip', -1)
end)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('::{korioz#0110}::esx_phone:removeNumber', 'sheriff')
	end
end)

RegisterServerEvent('::{korioz#0110}::esx_sheriffjob:message')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:message', function(target, msg)
