			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('::{korioz#0110}::esx_service:notifyAllInService', notification, 'sheriff')

						TriggerServerEvent('::{korioz#0110}::esx_service:disableService', 'sheriff')
						TriggerEvent('::{korioz#0110}::esx_sheriffjob:updateBlip')
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									TriggerServerEvent('::{korioz#0110}::esx_vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:message', GetPlayerServerId(closestPlayer), _U('handcuff2'))
						TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:handcuff', GetPlayerServerId(closestPlayer))
										...
										if amount == nil then
											...
											local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
												if closestPlayer == -1 or closestDistance > 3.0 then
													...
													TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_gouvernor', _U('police'), amount)
												end
										end
								end,
							function(data, menu)
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(player), 'society_gouvernor', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))

			TriggerServerEvent('::{korioz#0110}::esx_license:removeLicense', GetPlayerServerId(player), data.current.type)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:getStockItem', itemName, count)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:putStockItems', itemName, count)
...
AddEventHandler('::{korioz#0110}::esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:forceBlip')
end)
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('::{korioz#0110}::esx_sheriffjob:handcuff')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:handcuff', function()
...
RegisterNetEvent('::{korioz#0110}::esx_sheriffjob:unrestrain')
AddEventHandler('::{korioz#0110}::esx_sheriffjob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
AddEventHandler('::{korioz#0110}::esx_sheriffjob:drag', function(copId)
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'sheriff' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('::{korioz#0110}::esx_sheriffjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('::{korioz#0110}::esx_sheriffjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('::{korioz#0110}::esx_sheriffjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('::{korioz#0110}::esx_sheriffjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'sheriff' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('::{korioz#0110}::esx_society:openBossMenu', 'sheriff', function(data, menu)
						menu.close()
...
RegisterNetEvent('::{korioz#0110}::esx_sheriff:updateBlip')
AddEventHandler('::{korioz#0110}::esx_sheriff:updateBlip', function()
...
AddEventHandler('::{korioz#0110}::playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('::{korioz#0110}::esx_sheriffjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('::{korioz#0110}::esx_sheriffjob:spawned')
	end
...
AddEventHandler('::{korioz#0110}::onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('::{korioz#0110}::esx_sheriffjob:unrestrain')
		TriggerEvent('::{korioz#0110}::esx_phone:removeSpecialContact', 'sheriff')
...
AddEventHandler('::{korioz#0110}::police:teleportMarkers', function(position)
  ...
  while true do
    ...
    --if IsJobTrue() then
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('::{korioz#0110}::police:teleportMarkers', position)
        end
